...
import { formatDate, formatDateTime } from '@/lib/utils'; // formatDateTime importé
...
const AdminSupport = () => {
  const { user } = useAuth(); // Récupérer l'utilisateur admin connecté
  ...
  const [actionLoading, setActionLoading] = useState<string | null>(null); // Ajout de actionLoading
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      const updatedTicket = await ticketsApi.addMessage(selectedTicket.id, {
        ...
        authorId: user.id, // Utiliser l'ID de l'admin connecté
        authorName: `${user.firstName} ${user.lastName}`, // Utiliser le nom de l'admin connecté
        authorRole: 'admin'
      });

      // Mettre à jour le ticket sélectionné avec la nouvelle liste de messages
      setSelectedTicket(prevTicket => prevTicket ? {
        ...prevTicket,
        messages: updatedTicket.messages,
        updatedAt: updatedTicket.updatedAt, // Mettre à jour updatedAt aussi
        status: updatedTicket.status // Mettre à jour le statut si l'API le change
      } : null);

      setReplyMessage('');
      // Recharger la liste des tickets pour refléter la mise à jour (updatedAt, nouveau message count)
      ...
      if (ticketIndex !== -1) {
        ...
        await loadTickets(); // Fallback si le ticket n'est pas trouvé localement
      }
  ...
  const handleUpdateTicketStatus = async (ticketId: string, status: Ticket['status']) => {
    try {
      ...
      // Mettre à jour le ticket sélectionné s'il correspond
      ...
      toast({
        title: 'Succès',
        description: `Statut du ticket mis à jour en : ${status}`,
        variant: 'success'
      });
    } catch (error) {
      toast({
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-4 text-sm text-slate-600">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>Créé le {formatDate(new Date(ticket.createdAt))}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          ...
                          {selectedTicket && (
                              <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                  ...
                                  <div>
                                    <strong>Créé le:</strong> {formatDateTime(new Date(selectedTicket.createdAt))}
                                  </div>
                                  <div>
                                    <strong>Dernière mise à jour:</strong> {formatDateTime(new Date(selectedTicket.updatedAt))}
                                  </div>
                                </div>
                                <div className="mt-4">

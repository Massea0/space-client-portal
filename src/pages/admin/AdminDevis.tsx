...
import { contractsAI } from '@/services/contracts'; // Mission 1: Services IA pour contrats
...
import EditQuoteModal from '@/components/modules/quotes/EditQuoteModal';
import DeleteQuoteDialog from '@/components/modules/quotes/DeleteQuoteDialog';
...
const AdminDevisPage = () => {
  ...
  const fetchAllDevis = async () => {
    ...
    try {
      ...
      notificationManager.error("Erreur", { message: "Erreur lors du chargement des devis" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPDF = async (devis: DevisType) => {
    ...
    try {
      ...
      notificationManager.success("Succès", { message: `Le devis ${devis.number} a été téléchargé` });
    } catch (error) {
      console.error('Erreur lors du téléchargement du PDF:', error);
      notificationManager.error("Erreur", { message: "Échec du téléchargement" });
    } finally {
      setActionLoading(null);
    }
  };

  const handleCreateDevisSubmitInDialog = async (formData: DevisFormSubmitData) => {
    setIsSubmittingDevis(true);
    try {
      // Calculer le montant total en additionnant les montants des articles
      const amount = formData.items.reduce((sum, item) => sum + (item.quantity * item.unitPrice), 0);
      
      // Préparer les items au format attendu par l'API
      ...
      // Créer l'objet de données attendu par l'API
      const devisData = {
        object: formData.object || "Devis sans objet",
        companyId: formData.companyId || "",
        status: 'draft' as DevisType['status'],
        amount,
      ...
      fetchAllDevis();
      notificationManager.success("Succès", { message: "Devis créé avec succès!" });
    } catch (error) {
      console.error('Erreur lors de la création du devis:', error);
      notificationManager.error("Erreur", { message: "Erreur lors de la création du devis" });
    } finally {
  ...
  const confirmRejection = async () => {
    ...
    try {
      await devisApi.updateStatus(devisToUpdate.id, 'rejected', rejectionReason);
      fetchAllDevis();
      setIsRejectionDialogOpen(false);
      setRejectionReason('');
      notificationManager.success("Succès", { message: `Devis ${devisToUpdate.number} marqué comme rejeté` });
    } catch (error) {
      console.error('Erreur lors de la mise à jour du statut:', error);
      notificationManager.error("Erreur", { message: "Échec de la mise à jour du statut" });
    } finally {
      setActionLoading(null);
    }
  };

  const handleUpdateStatus = async (devisId: string, newStatus: DevisType['status'], reason?: string) => {
    console.log(`[DEBUG] handleUpdateStatus appelé avec:`, { devisId, newStatus, reason });
    ...
    try {
      if (newStatus === 'rejected') {
        ...
        console.log(`[DEBUG] Appel de devisApi.updateStatus pour ${devisId} vers ${newStatus}`);
        await devisApi.updateStatus(devisId, newStatus, reason);
        console.log(`[DEBUG] Mise à jour réussie, raffraîchissement des données`);
        fetchAllDevis();
        notificationManager.success("Succès", { message: "Statut mis à jour avec succès" });
      }
    } catch (error) {
      console.error('Erreur lors de la mise à jour du statut:', error);
      notificationManager.error("Erreur", { message: "Échec de la mise à jour du statut" });
  ...
  const handleConvertToInvoice = async (devis: DevisType) => {
    if (!devis || devis.status !== 'approved') { // Changed from 'validated' to 'approved' to match your status workflow
      notificationManager.error("Erreur", { message: "Le devis doit être approuvé pour être converti en facture" });
      return;
    }

    setActionLoading(devis.id);
    try {
      await invoicesApi.createFromDevis(devis.id);
      notificationManager.success("Succès", { message: `Le devis ${devis.number} a été converti en facture` });
      ...
      notificationManager.error("Erreur", { message: "Échec de la conversion en facture" });
    } finally {
      setActionLoading(null);
    }
  };

  // Mission 1: Génération de contrat IA
  const handleGenerateContract = async (devisId: string, devisNumber: string) => {
    const devis = devisList.find(d => d.id === devisId);
    if (!devis || devis.status !== 'approved') {
      notificationManager.error("Erreur", { message: "Le devis doit être approuvé pour générer un contrat" });
      return;
    }

    setActionLoading(`contract-${devisId}`);
    try {
      const contract = await contractsAI.generateContractDraft({
        devisId: devisId,
        clientId: devis.companyId, // Utiliser companyId comme clientId
        templateType: 'service' // Par défaut, peut être personnalisé
      });
      
      notificationManager.success("Succès", { 
        message: `Contrat généré avec succès pour le devis ${devisNumber}. Le contrat est disponible dans la section 'Contrats IA'`
      });
      
      // Optionnel: rediriger vers la page des contrats
      // navigate('/admin/contracts');
    } catch (error) {
      console.error('Erreur lors de la génération du contrat:', error);
      notificationManager.error("Erreur", { 
        message: `Échec de la génération du contrat: ${error.message || "Une erreur est survenue"}`
      });
    } finally {
      setActionLoading(null);
    }
  ...
  const renderDevisCard = React.useCallback((devis: DevisType) => {
    return (
      <InteractiveQuoteCard
        ...
        onViewDetails={(quote) => setSelectedDevis(quote)}
      />
    );
  }, [actionLoading, devisList, handleDownloadPDF, handleUpdateStatus, handleConvertToInvoice, handleEditDevis, handleDeleteDevis, setSelectedDevis]);

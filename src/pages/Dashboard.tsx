...
interface UserProfileForDashboard extends AuthUserType {
  createdAt?: Date;
}
...
const Dashboard = () => {
  ...
  const recentActivities = [
    ...devisList.slice(0, 2).map(d => ({
      ...
      type: 'devis' as const,
      title: d.object,
      description: `Devis ${d.number} - ${d.amount.toLocaleString('fr-FR')} FCFA`,
    ...
    ...invoices.slice(0, 2).map(i => ({
      ...
      description: `${i.amount.toLocaleString('fr-FR')} FCFA`,
        ...
        <div>
          ...
          <p className="text-slate-600 mt-2">
            {user?.role === 'admin'
                ...
                : `Bienvenue, ${user?.firstName} ${user?.lastName} - ${user?.companyName || 'Client'}`
            }
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="space-y-3">
              {/* TODO: Lier ces boutons à des actions réelles (ex: ouvrir le formulaire de création) */}
              {user?.role === 'admin' ? (
                  <>

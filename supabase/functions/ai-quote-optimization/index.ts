...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🎯 Analyse d'optimisation - Montant original: ${baseAmount} FCFA, Montant actuel: ${quote.amount} FCFA`);

    const analysisData: QuoteAnalysisData = {
      quote: {
        ...quote,
        amount: baseAmount, // Utiliser le montant original pour les calculs d'optimisation
        current_amount: quote.amount, // Garder le montant actuel pour référence
    ...
    return new Response(
      JSON.stringify({
        ...
        quote: {
          id: quote.id,
          originalAmount: quote.original_amount || quote.amount, // Utiliser le montant original
